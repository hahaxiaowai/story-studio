...
const { t } = useLocale()
const { activeWorkspace, setActiveWorkspaceStoryStyle } = useWorkspaces()
const {
  settings,
  providers,
  storyStyles,
...
const currentStoryStyle = computed(() => resolveStoryStyle(activeWorkspace.value.storyStyleId))
...
function updateWorkspaceStoryStyle(event: Event): void {
  setActiveWorkspaceStoryStyle(readEventValue(event))
}
...
function saveStoryStyle(): void {
  ...
  setActiveWorkspaceStoryStyle(style.id)
  resetStoryStyleDraft()
}

function deleteStoryStyle(styleId: string): void {
  if (activeWorkspace.value.storyStyleId === styleId)
    ...
    <div class="grid min-h-[34rem] lg:grid-cols-[18rem_minmax(0,1fr)]">
      ...
      <div class="grid content-start gap-5 p-5">
        ...
        <section class="grid gap-4 rounded-lg border p-4">
          <div>
            <h2 class="text-lg font-semibold">
              {{ t('assistant.storyStyles') }}
            </h2>
            <p class="text-muted-foreground mt-1 text-sm">
              {{ t('assistant.storyStylesHint') }}
            </p>
          </div>
          ...
          <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.8fr)]">
            <div class="grid gap-4">
              <label class="grid gap-1.5">
                <span class="text-muted-foreground text-sm">{{ t('assistant.currentWorkspaceStyle') }}</span>
                <select
                  class="border-input bg-background ring-offset-background focus-visible:ring-ring h-9 rounded-md border px-3 text-sm focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none"
                  :value="activeWorkspace.storyStyleId ?? ''"
                  @change="updateWorkspaceStoryStyle"
                >
                  <option value="">
                    {{ t('assistant.defaultStoryStyle') }}
                  </option>
              ...
              <div class="bg-muted/40 grid gap-2 rounded-md border p-3">
                <div class="flex items-center justify-between gap-2">
                  <h3 class="text-sm font-semibold">
                    {{ currentStoryStyle.name }}
                  </h3>
                  <span class="text-muted-foreground text-xs">
                    {{ currentStoryStyle.system ? t('assistant.builtInStoryStyle') : t('assistant.customStoryStyle') }}
                  </span>
                </div>
                <p v-if="currentStoryStyle.description" class="text-muted-foreground text-sm">
                  {{ currentStoryStyle.description }}
                </p>
                <p class="text-sm whitespace-pre-wrap">
                  {{ currentStoryStyle.constraints || t('assistant.storyStyleConstraintsEmpty') }}
                </p>
              </div>
            </div>
          ...
          <div class="grid gap-4 md:grid-cols-2">
            ...
            <label v-if="selectedProvider.kind === 'local-terminal'" class="grid gap-1.5 md:col-span-2">
              ...
              <Input
                ...
                placeholder="codex exec -m &quot;$STORY_STUDIO_MODEL&quot; -"
