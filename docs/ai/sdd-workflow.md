...
1. Spec 确认：Spec 定义目标、非目标、验收标准和验证方式；未确认前不进入实现。
2. Plan 确认：Plan 从 Spec 拆出可执行任务，记录任务验收标准、验证方式、依赖和预计影响文件。
3. Task 执行：按 Plan 逐任务推进，遵守依赖顺序，按 TDD 或 docs-only 验证执行，不扩大 Spec 范围。
4. 完成回填：完成后回填 Spec 的完成记录，并同步 Plan 状态、验证结果和必要文档。
