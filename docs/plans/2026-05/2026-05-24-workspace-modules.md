...
- 工作区状态已由后续持久化计划接入 `useStudioData()`，不再只存在于内存中。
- 当前计划用于记录已落地的实施边界，后续归档、重命名可在新规格中继续拆分。
...
4. 实现工作区组合式状态。
   ...
   - 通过 `useStudioData()` 读写 `workspaces` 和 `activeWorkspaceId`。
...
- 风险：当前状态只存在内存中，刷新后新建工作区会丢失。
  - 状态：已由 `docs/specs/2026-05-24-persistent-storage.md` 覆盖并实现。
