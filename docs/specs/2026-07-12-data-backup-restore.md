...
- [x] 先写失败测试：导出文件名和 JSON 内容稳定，且不修改源文档。
- [x] 先写失败测试：合法当前 schema 文档可以导入并生成摘要。
- [x] 先写失败测试：现有可迁移旧 schema 文档导入后升级到当前版本。
- [x] 先写失败测试：非法 JSON、非 Story Studio 对象、过旧和过新 schema 分别失败。
- [x] 先写失败测试：`replaceDocument()` 保存成功后替换当前文档。
- [x] 先写失败测试：`replaceDocument()` 保存失败时恢复旧文档并重新抛错。
- [x] 页面测试覆盖选择文件后只展示摘要，不自动恢复。
- [x] 页面测试覆盖明确确认后调用恢复并展示成功状态。

## 验收标准

- [x] 用户可以从顶部操作区导出当前完整数据。
- [x] 导出的文件可被当前应用重新选择、预览并恢复。
- [x] 可迁移旧文档导入后使用当前 schema。
- [x] 无效或不兼容备份不会覆盖当前文档。
- [x] 导入前必须展示摘要并二次确认。
- [x] 持久化失败时内存数据回滚。
- [x] 中英文界面文案完整，并提示备份可能含有 API Key。
- [x] `docs/features/storage.md` 与 `docs/plans/2026-07/` 已同步。
- [x] 聚焦测试、lint、typecheck、全量测试和 build 通过。
...
- [x] 更新 `docs/features/storage.md` 的用户入口、备份流程、错误边界和验证入口。
- [x] 新建 `docs/plans/2026-07/2026-07-12-data-backup-restore.md`。
- [x] 更新 `docs/plans/2026-07/TODO.md`。
- [x] 本次不需要 ADR：未改变单文档存储架构、运行时驱动边界或长期技术选型。

## 完成记录

- 完成时间：2026-07-12。
- 实际完成内容：已完成完整 JSON 导出、导入身份与版本校验、旧 schema 迁移、摘要预览、二次确认、原子持久化与失败回滚、顶部入口和中英文文案。
- 验证结果：目标测试 5 个文件、25 个测试通过；`pnpm run lint`、`pnpm run typecheck`、`pnpm run test`、`pnpm run build` 均通过；全仓 Studio 48 个测试文件、262 个测试通过；真实页面确认入口、摘要、敏感提示、确认门禁和关闭流程可用，控制台无 error/warn。
- commit：功能变更尚未提交，规格与计划提交分别为 `fb9f732`、`7716f0b`。
- 未覆盖风险：自动备份、历史版本、离线恢复、加密备份和 Tauri 多平台 WebView 下载行为不在本期验证范围内。
