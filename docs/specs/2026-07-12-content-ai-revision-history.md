...
- [ ] 先写失败测试：新建章节默认空历史。
- [ ] 先写失败测试：应用 AI 改写原子更新正文并追加完整记录。
- [ ] 先写失败测试：相同正文不新增记录。
- [ ] 先写失败测试：第 21 条记录淘汰最旧记录。
- [ ] 先写失败测试：恢复修改前正文并新增可反向恢复的记录。
- [ ] 先写失败测试：目标记录不存在或正文相同不产生变化。
- [ ] 先写失败测试：删除单条记录不修改正文或其他记录。
- [ ] 迁移测试覆盖 schema 13 补空数组、无效记录过滤和 20 条裁剪。
- [ ] `useContent` 测试覆盖原子应用、恢复和删除写回统一文档。
- [ ] 页面测试覆盖历史入口、记录数量、恢复/删除接线和即时撤销清理。

## 验收标准

- [ ] 每次实际应用 AI 改写后，当前章节新增一条持久历史。
- [ ] 历史包含要求、目标类型、修改前后整章正文和创建时间。
- [ ] 每章独立保留最近 20 条记录。
- [ ] 用户可以恢复任意记录的修改前正文，且恢复动作再次留痕。
- [ ] 用户确认后可删除单条记录，正文保持不变。
- [ ] 旧文档和旧备份迁移到 schema 14 后历史字段有效。
- [ ] 现有即时单次撤销继续可用，恢复历史后不会错误应用旧快照。
- [ ] 中英文文案、功能文档、月度计划和测试索引已同步。
- [ ] 目标测试、lint、typecheck、全量测试和 build 通过。
...
- [ ] 更新 `docs/features/content.md`。
- [ ] 更新 `docs/features/storage.md` 的 schema 和备份兼容边界。
- [ ] 新建 `docs/plans/2026-07/2026-07-12-content-ai-revision-history.md`。
- [ ] 更新 `docs/plans/2026-07/TODO.md` 和 `docs/ai/test-map.md`。
- [ ] 本次不需要 ADR：没有改变单文档存储架构或 AI Provider 边界。
