...
- 用户可以通过表单新建工作区，填写作品名称和简介，并自动切换到新建工作区。
- 用户可以在已有工作区之间切换。
...
- 本次不引入 Pinia 或路由系统。
- 本规格首版不为大纲、角色、地图、内容建立完整业务数据表；角色和大纲记录已在后续实体与属性模型规格中继续展开。
...
5. 用户点击“新建工作区”，系统弹出基础信息表单。
6. 用户填写作品名称和可选简介后提交，系统创建工作区并切换到该工作区。
7. 用户在当前工作区内访问大纲、角色、地图、内容。
8. 用户在公共功能区访问素材和助手。
...
- `Workspace`
  - `id`: 工作区唯一标识。
  - `title`: 作品名称。
  - `description`: 可选作品简介，用于记录工作区基础信息。
...
- `apps/studio/src/components/TeamSwitcher.vue`
  - 作为工作区切换器。
  - 显示当前工作区和新建工作区入口。
- `apps/studio/src/components/WorkspaceCreateDialog.vue`
  - 使用 shadcn-vue Dialog 表单收集作品名称和简介。
  - 作品名称必填，简介可选。
...
- 工作区状态通过 `useStudioData()` 读写统一持久化文档，不引入全局 store。
- 工作区创建时基于作品名称生成 id；重复标题会追加数字后缀，展示标题保持用户输入。
...
- 当前工作区数据已接入统一持久化文档；早期“前端内存状态”只作为首版历史背景保留。
- 工作区重命名和归档应在后续规格中单独展开。
- 角色、大纲记录和可配置属性见 `docs/specs/2026-05-27-entity-property-model.md`。
